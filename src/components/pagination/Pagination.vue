<template>
  <nav aria-label="...">
    <ul class="pagination">
      <li @click="onPrev()" class="page-item">
        <button :disabled="this.$store.state.currentPage === 1" class="page-link" tabindex="-1">
          Previous
        </button>
      </li>
      <li
        v-for="(page, index) of getpages()"
      ...
      <li @click="onNext()" class="page-item">
...
export default {
  ...
  methods: {
    isOnLastPage() {
      return (
        this.$store.state.perPage * this.$store.state.currentPage >=
        this.$store.getters.totalItemCountAfterFilter
      )
    },
    totalPages() {
      return (
        Math.ceil(this.$store.getters.totalItemCountAfterFilter / this.$store.state.perPage) || 0
      )
    },
    getMin() {
